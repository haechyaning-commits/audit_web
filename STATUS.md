...
### 다음
- [ ] 데이터 품질 2차 수정 + DB 반영 끝나면 실제 백엔드와 연결해서 재검증
- [ ] 4주차 배포 시 `VITE_API_BASE_URL`(프론트), `FRONTEND_ORIGIN`(백엔드) 환경변수 설정
- [ ] README.md 스크린샷 자리에 실제 화면 캡처 채워넣기

---

## 🔧 데이터 품질 사고 대응 (2026-08-07, 2주차 이후)

**검색 결과 미리보기에서 텍스트 오염 발견 → 원인 진단 → 수정 → 라이브 DB 반영까지.**
`documents.raw_text` / `chunks.text`는 완료, `chunks.embedding` 재임베딩은 진행 중.
...
### 다음
- [x] `documents.raw_text` 반영 + 라이브 재검증(0/0)
- [x] `chunks.text` 반영 + 라이브 재검증(0/0)
- [ ] 바뀐 chunk 4,770건 BGE-m3 재임베딩(Colab) 후 `chunks.embedding` UPDATE — 텍스트만
      고치고 벡터를 안 바꾸면 검색은 여전히 옛날 텍스트 기준으로 동작하므로 필수
- [ ] `/search` 실제 호출로 최종 확인
