...
## 🔧 데이터 품질 사고 대응 (2026-08-07, 2주차 이후) — 진행 중, DB 반영은 끝남·마무리 조사 남음

**검색 결과 미리보기에서 텍스트 오염 발견 → 원인 진단 → 수정 → 라이브 DB 반영 → 실제
검색 결과로 재확인 → 잔여 이슈 2개 추가 발견 → 2차 수정 코드 작성 → 2차 수정 DB 반영
+ 재임베딩 + HNSW 인덱스 재생성까지 전부 완료(2026-08-10, 중간에 디스크 부족 2번 겪었지만
해결) → 재추출 불가 문서 건수 급증(1,765 예상 vs 5,162 실제) 원인도 조사 완료, 버그
아니라 진짜 노이즈였음을 확인 → **삭제까지 실행 완료**(documents 5,162건 + chunks 7,054건
제거, 2026-08-10). 남은 건 라이브 검색 최종 확인과 문서 수 표기 갱신뿐.**

### ⏭️ 다음 세션은 여기부터
1. **README.md 등 총 문서 수 표기 갱신** — 현재 72,913건으로 적혀있는데, 5,162건 삭제로
   **약 67,751건**이 됨. `architecture.md`/`development-plan.md`에도 72,913 언급 있는지
   확인해서 같이 갱신할 것
2. 라이브 DB 재검증 + "폭력" 검색으로 `22002244년도`(인천교통공사 사례)가 `2024년도`로,
   "표" placeholder 잡음이 사라졌는지 최종 확인 — Railway 서비스가 비용 관리로 삭제되어
   있는 상태라 재배포할지 로컬에서 확인할지 아직 미정, 정해야 함
3. 그 다음에야 진짜 완료 — 2026-08-07에 시작된 데이터 품질 사고 대응 전체가 마무리됨
