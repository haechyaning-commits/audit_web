...
2. **URL 구조 개선 1단계** — 어제 항목 2번 착수.
   ...
   - 프론트: `/cases/:id/:slug` 라우트 추가(`caseUrl.js`). **`/cases/기관명/연도/제목`처럼
     ...
     조용히 새 URL로 교체함.
   - **다음에 이어서**: 상세페이지 `<title>`(브라우저 탭)/메타태그에 실제 제목 반영 안
     했음(지금은 고정 타이틀) — SEO/공유 미리보기까지 신경쓰려면 추가 필요. 실제 배포
     환경(Vercel)에서 구 URL(`/documents/:id`) 공유 링크가 새 URL로 잘 리다이렉트되는지,
     검색결과 카드 클릭 시 새 URL로 바로 뜨는지 눈으로 한 번 확인 안 함.
...
3. **URL 구조 개선 마무리** — 위 "다음에 이어서" 참고(탭 타이틀/메타태그, 실배포 확인).
