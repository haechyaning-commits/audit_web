...
# 확인하도록 함.
#
# **2026-08-18: 첫 DRY_RUN 결과(2단 레이아웃 후보 2,137건) 진단** — 자동반영
# 674건 / 수동검토 1,463건(68.5%). 수동검토 큐 샘플 4건을 실제 DB원문과 대조해
# 원인을 3가지로 특정함(진단 스크립트: scripts/diagnose_pdf_reextract_review_queue.py):
#   1) 마스킹 기호 정규화 누락 — ☆☆병원, ▽▽과처럼 원래 5종(○◎□●■) 밖의 기호가
#      안 걸러짐 → 이번에 별/삼각형 계열(☆★△▲▽▼◇◆)까지 넓혀서 고침.
#   2) 선행 상용구 미제거(위 경고 1번, 여전히 미해결) — "현지조치 요구서..." 같은
#      표지 문구가 재추출 결과에 그대로 남아 DB 원문(제목부터 시작)과 크게 벌어짐.
#   3) [신규 발견, 미해결] 폰트/서브셋 경계에서 숫자·괄호 조각이 엉뚱한 위치로
#      밀려나는 경우 있음(예: "제4조(감사의 구분)에" -> "제 조 감사의 구분 에 4 ( )").
#      _merge_same_row_fragments()의 엣지케이스로 추정 — 원인은 특정했지만 고치면
#      다른 정상 케이스를 깨뜨릴 위험이 있어 지금은 손대지 않음. 유사도 게이트가
#      이런 문서를 정확히 걸러내고 있으므로(DB 오염 없음) 급하지 않음.
# 결론: 68.5% 탈락은 게이트가 오작동한 게 아니라 실제 품질 문제를 정확히 걸러낸
# 것으로 확인됨(1번만 고치고 2·3번은 알고 있는 채로 다음 세션/사람 판단으로 넘김).
#
# **실행 순서**:
...
# ------------------------------------------------------------------
# 2026-08-18: DRY_RUN 유사도 진단 중 ☆☆병원, ▽▽과 같이 원래 5종 밖의 기호로
# 마스킹된 문서를 발견해서 별/삼각형 계열까지 넓힘 (reextract_pdf_text.py와 동일)
