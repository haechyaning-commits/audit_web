...
- 백엔드 API: `[ ]` (Railway 무료 티어 특성상 공개 도메인이 재배포될 때마다 바뀜 — GitHub 연동을 재연결할 때마다 새 도메인이 발급되는 게 원인. 커스텀 도메인을 아직 안 걸어놔서 현재 값은 Railway 대시보드에서 직접 확인 후 채워 넣을 것. `/health`, `/search?q=`, `/documents/{id}`, `POST /documents/{id}/summary` 엔드포인트가 있음)

---

## 왜 만들었나

감사실 인턴으로 일하며 겪은 불편함에서 출발한 프로젝트입니다. "이런 사안, 예전에 어떻게 처리됐지?"라는 질문이 실무에서 자주 나오는데, 정작 과거 감사 사례를 찾으려면 문서를 하나씩 열어보는 것 말고는 뾰족한 방법이 없었습니다. 원문 자체도 정형화된 서류 양식(제목/관계기관/처분요구 등 라벨 뒤에 내용이 이어지는 구조)이라 사람이 읽기엔 익숙하지만, 키워드 검색만으로는 "출장비 정산 관련 지적 사례" 같은 자연어 질문에 맞는 결과를 찾기 어려웠습니다.
...
`[ ]` 배포 안정화(백엔드 도메인 확정) 이후 실제 검색화면/상세페이지 스크린샷 첨부 예정
...
**실제로 동작하는 것**:
- 자연어 질문 입력 → 하이브리드 검색(벡터 검색 + 키워드 검색을 RRF로 융합) → 문서(사례) 단위로 중복 제거해 Top-40 카드를 2열 그리드+페이지네이션으로 표시
...
**시간 관계상 못 넣은 것(설계는 돼 있음, 아래 "한계와 다음 계획" 참고)**: 리랭커(크로스인코더 2차 재채점), 한국어 형태소 토큰화(kiwipiepy), 정식 검색품질 평가(eval set). 대신 이 시간을 **원본 데이터 파싱 품질**에 투자했습니다 — 아래 "데이터 품질 문제 대응" 참고.
...
**스트레치 목표로 미룬 것(설계는 완료)**:
- 리랭커(bge-reranker-v2-m3) 2차 재채점 — RRF만으로도 검색은 동작하지만, 순위 기반 융합 특유의 한계(둘 다 하위권인 후보 과대평가)는 아직 그대로
- 한국어 형태소 토큰화(kiwipiepy) — 키워드 검색 leg의 조사/어미 매칭 정확도 개선
- 정식 검색품질 오프라인 eval set(Recall@10/MRR, RRF vs 벡터단독 vs 키워드단독 비교)
