...
> **재현성에 대해 정직하게**: 위 eval set(쿼리-정답 문서 쌍)은 재현용으로 저장소에 커밋돼 있지 않습니다(`scripts/eval_set_template.jsonl`은 쿼리만 있는 템플릿, 실제 정답 라벨은 없음) — `scripts/eval_search_quality.py` 자체는 있지만 그걸 돌린 원본 데이터가 없어서 제3자가 위 수치를 그대로 재현할 수는 없습니다. 실제로 별도 세션에서 22개 쿼리로 eval set을 새로 구성해 리랭커 비교를 재현해봤는데, 그때는 MRR이 0.401→0.404로 거의 변화가 없었습니다(-23% 악화가 재현되지 않음) — 표본이 20여 개로 작아서 eval set 구성에 따라 절대 수치가 크게 흔들릴 수 있다는 뜻입니다. "리랭커 미적용" 결론(RRF만으로 충분, 메모리만 더 쓰고 효과는 불확실) 자체는 유효하지만, 위 구체적인 수치는 참고용 방향성으로만 봐주세요.
